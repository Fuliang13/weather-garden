...
describe("Meteo-France ODIM raw attribute recovery", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  ...
  it("recovers ODIM attributes from raw metadata groups when regular header parsing misses them", async () => {
    const hdf5Fixture = await buildOdimMetadataHdf5Fixture({ hideMetadataAttributesFromHeaders: true });
    ...
    const whereGroup = radar.diagnostics.hdf5.groups.find((group) => group.path === "/where");
    const whatGroup = radar.diagnostics.hdf5.groups.find((group) => group.path === "/what");

    expect(radar.nativeLayer.ok).toBe(true);
    ...
    expect(whereGroup.attributes.some((attribute) => attribute.name === "projdef" && attribute.recovered === true)).toBe(true);
    expect(whatGroup.attributes.some((attribute) => attribute.name === "quantity" && attribute.recovered === true)).toBe(true);
    expect(radar.diagnostics.hdf5.structure.recoveredOdimAttributeCount).toBeGreaterThan(0);
    expect(radar.diagnostics.hdf5.structure.recoveredOdimAttributePaths).toContain("/where");
    expect(JSON.stringify(radar)).not.toContain("api-key-token");
    expect(JSON.stringify(radar)).not.toContain("secret-product-token");
  });
});

async function buildOdimMetadataHdf5Fixture({ hideMetadataAttributesFromHeaders = false } = {}) {
  ...
  if (hideMetadataAttributesFromHeaders) {
    writeU16(view, whereHeaderAddress + 2, 1);
    writeU16(view, whatHeaderAddress + 2, 1);
  }
