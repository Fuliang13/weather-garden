...
describe("Meteo-France native HDF5 raster layer", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  ...
  it("builds a Leaflet image overlay only when HDF5 grid values are decoded", async () => {
    const hdf5Fixture = await buildDecodableHdf5Fixture();
    ...
    expect(radar.nativeLayer.imageDataUrl).toMatch(/^data:image\/png;base64,/);
    expect(radar.frames).toHaveLength(1);
    expect(radar.frames[0]).toMatchObject({
      provider: "meteofrance-radar",
      bounds: [[48.1, -1.5], [48.9, -0.7]],
      width: 868,
      height: 868,
...
async function buildDecodableHdf5Fixture() {
  ...
  const heapData = buildHeapData(["", "data1"]);
  ...
  writeChunkBtreeLeaf(bytes, view, chunkBtreeAddress, [
    {
      byteLength: compressedChunk.length,
      filterMask: 0,
      offsets: [0, 0],
      address: chunkDataAddress
    }
  ], 2);
